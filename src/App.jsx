...
/* === Round 120: 取得範囲拡張 (データ蓄積の土台) ===
   従来は発走±60分の最大 12 レースしか出走表/オッズを取得しなかったため、
   朝アプリを開いても予想されるレースが極端に少なかった。
   Round 120 で 「今日の残り全レース」 を対象に拡張する。
   ・出走表は当日全レースで取得 (最大 144 レース、 並列度 4)
   ・オッズ + 直前情報は発走 3 時間前以内のレースで取得
   ・結果は発走から終わったレースで取得
   ・boatrace.jp に過度な負荷をかけないよう並列度制限あり */
const FULL_DAY_FETCH_CONCURRENCY = 4;
const ODDS_FETCH_WINDOW_MIN = 180; // 発走 3 時間前から オッズ取得開始

/* 並列度制限のヘルパー (workerプール方式) */
async function withConcurrency(items, fn, n) {
  ...
  const concurrency = Math.min(Math.max(1, n), Math.max(1, items.length));
...
export default function App() {
  ...
  const refreshAll = useCallback(async () => {
    ...
    /* ② Round 120: 当日全レースを対象に拡張 (旧: 発走±60分の最大 12 レース)
       ・出走表は全レースで取得 (毎日約 144 レース)
       ・オッズ + 直前情報は発走 3 時間前以内
       ・結果は発走から少し経過した終了レース
       並列度 4 で順次取得 → boatrace.jp への過度な負荷を避ける */
    const now = Date.now();
    const candidates = baseRaces.filter((r) => {
      const e = startEpoch(r.date, r.startTime);
      if (e == null) return false;
      const diff = (e - now) / 60000;
      // 過去 60 分以内 (= 終了直後で結果反映が必要) 〜 当日のすべての未来レース
      return diff >= -60;
    });
    setRefreshMsg(`🔄 確認中… (候補 ${candidates.length} レース / 並列度 ${FULL_DAY_FETCH_CONCURRENCY})`);

    const dateK = todayKey();
    const enriched = await withConcurrency(candidates, async (r) => {
      const e = startEpoch(r.date, r.startTime);
      const minutesToStart = e != null ? (e - now) / 60000 : null;
      const finished = e != null && now > e + 5 * 60 * 1000;
      // オッズは発走 3 時間前以内のみ取得 (それ以前は boatrace.jp に未掲載)
    ...
    const enrichedMap = Object.fromEntries(
      enriched.filter((e) => e && e.id && !e.error).map((e) => [e.id, e])
    );
