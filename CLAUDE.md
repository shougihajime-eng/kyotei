...
- ✅ Round 175 — **ホーム強化 (ヒーロー表現 + 理由 3 行)** (SPEC §0.1, §4, §8)。 ① ブランドバナー直下の 2 個の SumBox (激荒れ/荒れ注意) を Hero コンポーネントに統合。 「今日の勝負」 を超大文字 (44px) で表示、 アラーム時は赤グラデ + glow 18px、 注意時は橙、 0 件は控えめに「🌙 現時点で勝負レースなし」。 サブで内訳 (激荒れ N / 荒れ注意 N)、 「↓ 下にスクロールして買い目を確認」 ヒント。 ② RaceCard の理由表示を 1 行スラッシュ連結 → 3 行リスト (各行頭に色付き •) に。 「💡 なぜこのレースか」 のラベル付き。 SPEC §4 「折りたたまずに常時表示」 を遵守。 mansyu.js に `buildMansyuReasonLines(scoreResult, max=3)` を新規 export (旧 `buildMansyuReason` は後方互換のため残置)。
- 🟡 進行中: なし
- 🔜 次の一歩: **SPEC §9 ロードマップに従って Round 176 から実装**。 ① Round 176: 「🔬 研究所」 タブの中身整理 (AI 根拠 / 学習履歴 / モーター傾向 / 会場傾向 / 万舟分析) + 入口を小さくする → ② Round 177: 通知システム土台 → ③ Round 178: スマホ最適化 → ④ **Round 179-181: 場別ランキング機能** → ⑤ Round 182: AI 段階 B 細粒度学習。
