...
- ✅ 直近で済んだこと: **🧹 万舟研究所 フレッシュスタート完了** — 旧競艇アプリ (Round 120-160) のデータを完全消去。 ① localStorage は "kyotei" / "manfune" で始まる全キー + sessionStorage を起動時に強制削除 (`manfuneFreshStartDone` フラグ)、② Supabase の `kyotei_app` スキーマを `DROP CASCADE` (predictions 1656 件 / race_outcomes 361 件 / race_results 420 件 / races 552 件 / recommended_tickets 1444 件 ほか合計 5000 件超を削除)、③ ローカル `dist/` / `node_modules/.cache` / `.vite` を削除。 `hissatsu` (1号艇研究所) や他プロジェクトのスキーマには触れていない。
- 🟡 進行中: なし
- 🔜 次の一歩: 「万舟研究所」 のコンセプト・予想思想・UI 方針を決める → 新スキーマ (`manfune_lab` 等) を設計 → アプリ名 / 配色 / メイン画面の刷新。
