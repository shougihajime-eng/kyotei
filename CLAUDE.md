...
- ✅ 直近で済んだこと: **Round 120-135 完了**。120-133 予想精度系、 134 = ネオン光り感、 **135 = 予想を時系列で追える機能** — predictions に history 配列を追加 (重要な変化があった時だけスナップショット、 最大 10 件)、 RaceDetail に「📈 予想推移」 カード追加。 朝・直前・締切前の予想がどう変わったか時系列で見える。
- 🟡 進行中: なし
- 🔜 次の一歩: 本番で確認 → 1 週間実運用してデータ蓄積観察 → 30 件超えたら学習機能 → Round 136 (学習機能拡張) → Round 137 (機械学習化)。
