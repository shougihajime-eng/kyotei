...
- ✅ 直近で済んだこと: **Round 120-124 完了** (深夜一気書き)。Round 120 = 全レース取得拡張、 Round 121 = 選手コース別実績 (進入率/3連対率) 反映、 Round 122 = 進入予想 + 戦法相性強化、 Round 123 = 直近 3 節成績で好調/不調補正、 **Round 124 = 危険レース判定の重み付け化 + F・L 多発 / 1 号艇不調 / データ欠損 を追加**。 計画書は `docs/ROADMAP-WORLD-CHAMPION.md`。
- 🟡 進行中: なし
- 🔜 次の一歩: 1 週間実運用してデータ蓄積を観察 → 30 件超えたら学習機能が動き始める → Round 125 (学習機能の 5 因子全部への拡張) → Round 126 (機械学習化)。
