...
- ✅ 直近で済んだこと: **Round 120-125 完了** (深夜一気書き)。Round 120 = 全レース取得拡張、 121 = 選手コース別実績反映、 122 = 進入予想 + 戦法相性強化、 123 = 直近 3 節成績補正、 124 = 危険レース判定強化、 **125 = 整合性総チェック (boatsSnapshot に courseStats / recentForm / toban を追加 → 振り返り可能化)**。 計画書は `docs/ROADMAP-WORLD-CHAMPION.md`。
- 🟡 進行中: なし
- 🔜 次の一歩: 1 週間実運用してデータ蓄積観察 → 30 件超えたら学習機能 (5 因子) が動き始める → Round 126 (学習機能拡張) → Round 127 (機械学習化)。
