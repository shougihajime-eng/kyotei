...
- ✅ Round 187 — **第 3 層: シャドーモード (最低限版) 実装** (SPEC §13.3)。 シャドーストレージ追加、 学習結果は本番に即反映せず 7 日待ち昇格。
- ✅ Round 187.5 — **シャドー比較を ROI ベースに強化** (SPEC §13.3 完成版)。 ① `mansyuBacktest.js` に `rebuildRaceFromSnapshot(entry)` (snapshot から擬似 race オブジェクト構築) と `runBacktestWithWeights({days, jcd, weights})` (任意の重みで snapshot を再評価し、 show 判定エントリの virtualPnl を集計) を追加。 ② `mansyuLearningAuto.js` の `checkAndPromoteShadows` を強化 — 7 日経過したシャドーに対し `evaluateShadowVsProduction(production, shadow, jcd)` で本番 vs シャドーの ROI を比較: シャドー ≥ 本番 +5pt → **昇格** / シャドー < 本番 +5pt → **破棄** (clearShadowWeights + 履歴に shadow_rejected) / サンプル < 5 件 → 14 日まで待機 → それでも不足なら時間ベース昇格 (フォールバック)。 場別シャドーも同じ判定。 ③ ResearchOverview の KIND_META に `shadow_rejected` / `venue_shadow_rejected` (🚫 赤) 追加、 Coming soon の「シャドー成績比較 (Round 187.5)」 を削除 (完了)。 履歴エントリに metrics (productionROI / shadowROI / サンプル数) も保存。 これで「成績が良くなったと客観確認できたシャドーだけ本番昇格、 良くないシャドーは破棄」 の完全な安全設計が完成 (= shoug 必須要件 3 つすべて達成)。
- ✅ Round 188 — **ホーム UI 大改修「本日の勝負レース」を最上部に超大判表示** (SPEC §0.1, §4, §8)。 「余白多すぎ・文字小さい・買い目が分かりにくい」 という指摘を受け、 ホーム最上部の構成を全面刷新。 ① 新規 `src/components/PrimaryBattleCard.jsx` (約 320 行) — 締切最短かつ最高スコアの 1 レースを画面いっぱいの大判ヒーローで表示。 場名 + R番 (clamp 28→60px)・締切バッジ (5 分以内点滅)・荒れスコア (clamp 48→68px・glow)・万舟期待度 ★・💡 なぜこのレースか 3 行 (clamp 15→17px)・💰 今日はこれを買う (combo clamp 28→36px・stake clamp 18→22px・合計 5,000 円バッジ大)・🏁 結果 + 収支 (確定後・mansyuSkipLog から virtualPnl を取得して緑/赤表示)。 ② 新規 `src/components/MonitorStatusBar.jsx` (約 200 行) — 「本当に裏で動いてるのか」 を見える化する細い帯。 🟢 監視中 LED (呼吸アニメ `mansyu-led`)・対象 5 場巡回中 (戸田・江戸川・平和島・鳴門・桐生)・本日 N 判定 (🎯買 M / 🌙見送 K / 🏁確 L)・最終更新時刻 + 経過秒・次回更新まで N 秒・🔄 今すぐ更新ボタン。 ③ MansyuTop.jsx 全面再構成 — 旧 Hero/UpdateStatus/TapButton を削除 (MonitorStatusBar に統合)、 ブランドバナーをコンパクト化 (タイトル 1 行 + 勝負 N レース バッジ)、 PrimaryBattleCard を最上段、 残りの勝負レースは「📋 そのほかの勝負レース」 として CardGrid 縦並び。 ④ index.css に `@keyframes mansyu-led` 追加 (2 秒呼吸)。 ⑤ ビルド 931 modules / 12.2s 通過 (TDZ audit パス)。 自動監視ロジック (`refreshAll` → 5 場 filter → 並列 fetchProgram/Odds/Before/Result/Forecast → setRaces → MansyuTop useEffect で recordBatch + attachResults + attachBuyOrders) は SPEC 通り動作確認済み — 30 秒インターバル + visibility/focus 復帰 catch-up で、 ブラウザを開いている間は 8-22 時に自動巡回。
- ✅ Round 189 — **常時監視ロードマップ 起案** (SPEC §14 確定)。 「アプリ閉じてても動く」 仕組みの全体設計を 1 枚にまとめた。 ① 新規 `docs/ROADMAP-ALWAYS-ON.md` (約 190 行) — Vercel Cron + Supabase + Web Push の 3 本柱、 必要テーブル設計 (push_subscriptions / cron_log / skip_log.notified_at)、 Service Worker 設計、 VAPID キー運用、 Round 190-194 のフェーズ分け、 リスクと対策表、 費用試算 ($20/月 Vercel Pro)。 ② `docs/SPEC.md` に §14 を追記 — 3 つの柱・必須条件 (Vercel Pro / VAPID 環境変数 / iOS PWA)・新規テーブル・やらないこと・完成イメージ。 ③ SPEC のロードマップ表に Round 188-194 を追加。 **実装はユーザーの課金判断待ち** (Vercel Pro $20/月の承認が必要)。
- ✅ Round 190 — **「本日の勝負レース」 を TOP3 並列表示に拡張** (SPEC §0.1, §4)。 「1 件大判 + 残りは小カード」 から 「TOP3 を同じ大きさで並べる」 に変更。 ① MansyuTop の primary 抽出を `top3 = [...alarms, ...warns].slice(0, 3)` に変更、 4 件目以降は「📋 そのほかの勝負レース」 へ。 ② grid `repeat(auto-fit, minmax(min(100%, 420px), 1fr))` で 1 件は全幅・ 2 件は PC 2 列・ 3 件は PC 3 列 (ワイド時)・スマホは縦並びに自動切替。 ③ PrimaryBattleCard の clamp 上限を一段下げて TOP3 並列でも収まるよう調整。 ④ marginBottom を外して grid gap と二重 spacing 解消。 ⑤ ビルド 931 modules / 9.5s 通過。
- ✅ Round 190.5 — **「📋 一覧」 タブ復活** (SPEC §6)。 ユーザー指摘「予想してないだろ」 を受けて、 Round 171 で削除した「📋 一覧」 を別仕様で再導入。 ① 新規 `src/components/RaceList.jsx` (約 290 行) — 5 場全レースを 1 行ずつ横並びで表示。 1 行 = 場名 + R番 + 締切バッジ + 荒れスコア + レベルラベル + 万舟期待度 ★ + 買い目 (上位 2 点 + 残数 + 合計 5,000 円) + 結果 (終了時) + 「🔬 詳しく」 ボタン。 ② 並び替え 2 種 (締切順 / スコア降順)、 フィルター 2 種 (締切前のみ / 全部) の SegGroup スイッチ。 ③ alarm 行は赤グラデ + glow、 warn 行は橙、 終了行は opacity 0.7 で半透明化。 ④ Header.jsx の TABS に list を追加 (4 タブ構成に復帰)、 App.jsx に tab === "list" レンダリングを追加。 ⑤ SPEC §6 の「完全廃止」 を「Round 190.5 復活」 に書き換え、 ロードマップ表に Round 190 / 190.5 を「済」 として追加、 Round 190/191 (Vercel Cron / クラウド主データ化) は番号衝突回避のため 191/191.5 にずらした。 → ホーム = 勝負レースのみ大判 / 一覧 = 全レース横並びの予想ビュー で棲み分けが完成。
- 🟡 進行中: なし
- 🔜 次の一歩: **常時監視 設計のまま保留中** (2026-05-11 確認)。 確定事項: ✅ iPhone でも通知を受けたい → Round 192 で PWA インストール導線を組む前提。 保留事項: 🟡 Vercel Pro $20/月の課金 (shoug さん「まだ決められない」)。 OK になったら Round 190 から着手 (8-12 日で完成見込み)。 残 (任意): Round 200 想定: 段階 C ディープラーニング、 モーター別 / 選手別 細粒度学習、 大規模 PC 2 カラムレイアウト。
